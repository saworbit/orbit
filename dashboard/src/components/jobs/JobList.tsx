...
import { Play, X, Trash2, Search, Filter, RefreshCcw, Network } from "lucide-react";
...
export default function JobList({ compact = false }: JobListProps) {
  ...
  const filteredJobs = useMemo(() => {
    ...
    return jobs.filter((job) => {
      const matchesStatus = filter === "all" || job.status.toLowerCase() === filter;
      const matchesSearch =
  ...
  const getStatusBadge = (status: string) => {
    const styles: Record<string, string> = {
      ...
      running: "bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400 border-blue-200 dark:border-blue-800",
    ...
    return (
      <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold border capitalize ${styles[status.toLowerCase()] || styles.pending}`}>
        {status}
      </span>
    );
  };
        ...
        <>
          <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Monitor and manage your data transfer operations</p>
            </div>
          </div>
        ...
        {filteredJobs.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="text-muted-foreground">
              {search || filter !== "all" ? "Try adjusting your filters" : "Create a new job to get started"}
            </p>
          </div>
        ) : (
              ...
              <div key={job.id} className="p-4 hover:bg-accent/50 transition-colors">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-muted-foreground mb-3">
                  ...
                  <div className="flex gap-1 truncate" title={job.destination}>
                    <span className="font-semibold">Dst:</span> {job.destination}
                  </div>
                </div>
                ...
                {job.status === "running" && (
                  <div className="space-y-1.5 mb-3">
                    ...
                    <div className="flex justify-between text-[10px] text-muted-foreground">
                      ...
                      <span>{(job.completed_chunks / (job.total_chunks || 1) * 100).toFixed(1)}% complete</span>
                    </div>
                  </div>
                )}
                ...
                <div className="flex justify-end gap-2 mt-2">
                  {job.status === "pending" && (
                    <ActionBtn onClick={() => runJob.mutate(job.id)} icon={Play} label="Run" color="text-green-500 hover:bg-green-500/10" />
                  )}
                  {job.status === "running" && (
                    <ActionBtn onClick={() => cancelJob.mutate(job.id)} icon={X} label="Cancel" color="text-orange-500 hover:bg-orange-500/10" />
                  )}
                  <ActionBtn onClick={() => deleteJob.mutate(job.id)} icon={Trash2} label="Delete" color="text-red-500 hover:bg-red-500/10" />
                </div>
              </div>
            ))}
          </div>
