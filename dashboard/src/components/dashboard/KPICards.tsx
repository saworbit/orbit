...
export function KPICards() {
  ...
  const totalChunks =
    jobs?.reduce((sum, j) => sum + (j.completed_chunks || 0), 0) || 0;
  ...
  const avgProgress =
    runningJobs.length > 0
      ? (
          runningJobs.reduce((sum, j) => sum + (j.progress || 0), 0) /
          runningJobs.length
        ).toFixed(1)
      : "0";
        ...
        return (
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div
                className={`w-12 h-12 rounded-lg flex items-center justify-center ${colorClasses[kpi.color as keyof typeof colorClasses]}`}
              >
