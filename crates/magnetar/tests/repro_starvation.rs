...
#[tokio::test]
async fn test_parallel_compute_basic_functionality() {
    // Test that parallel compute works correctly (functionality test, not starvation test)
    use magnetar::executor::offload_parallel_compute;

    println!("Testing parallel compute executor basic functionality...");

    // Create 4 simple tasks to verify parallel execution works
    let tasks = vec![1, 2, 3, 4];

    let results = offload_parallel_compute(tasks, |x| -> anyhow::Result<u64> {
    ...
    println!("Parallel executor finished. Results: {:?}", results);
