...
struct Cli {
    ...
    #[arg(short, long, value_name = "TYPE", default_value = "none")]
    compress: String,
    ...
    #[arg(short = 'L', long, value_enum, default_value = "preserve")]
    symlinks: SymlinkModeArg,
    
    /// Copy mode
    #[arg(short = 'm', long, value_enum, default_value = "copy")]
    mode: CopyModeArg,
...
fn run() -> orbit::error::Result<()> {
    ...
    if !cli.source.exists() {
        eprintln!("Error: Source path does not exist: {:?}", cli.source);
        process::exit(1);
    }
    
    // Determine if source is file or directory
    let source_metadata = std::fs::metadata(&cli.source)?;
    ...
    if source_metadata.is_dir() {
        ...
        println!("📂 Copying directory tree: {:?} -> {:?}", cli.source, cli.destination);
        
        let stats = copy_directory(&cli.source, &cli.destination, &config)?;
        ...
        println!("📄 Copying file: {:?} -> {:?}", cli.source, cli.destination);
        
        let stats = copy_file(&cli.source, &cli.destination, &config)?;
