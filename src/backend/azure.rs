//! Azure Blob Storage backend implementation via `object_store`
//!
...
impl AzureBackend {
    ...
    /// Convert a Path to an Azure blob name. object_store's list APIs are
    ...
    fn path_to_blob_name(&self, path: &Path) -> ObjectPath {
        ...
        let full_path = match &self.prefix {
            Some(p) => format!("{}/{}", p.trim_end_matches('/'), blob_name),
...
impl Backend for AzureBackend {
    #[tracing::instrument(
        skip(self),
        fields(otel.kind = "client", backend = "azure", path = %path.display())
    )]
    ...
    #[tracing::instrument(
        skip(self, options),
        fields(otel.kind = "client", backend = "azure", path = %path.display(), recursive = options.recursive)
    )]
    async fn list(&self, path: &Path, options: ListOptions) -> BackendResult<ListStream> {
        ...
        if options.recursive {
            // Drive the underlying borrow-from-self list stream on a spawned
            // task and forward through a bounded channel so the returned
            // `ListStream` is `'static` and lazily backpressured.
    ...
    #[tracing::instrument(
        ...
        fields(otel.kind = "client", backend = "azure", path = %path.display(), overwrite = options.overwrite, size_hint = ?size_hint)
    )]
    async fn write(
        &self,
        path: &Path,
        ...
        // Streaming multipart upload. Conditional-create has no equivalent
        // on the multipart path; emulate with a best-effort head() probe.
    ...
    #[tracing::instrument(
        skip(self),
        fields(otel.kind = "client", backend = "azure", path = %path.display(), recursive)
    )]
    ...
    #[tracing::instrument(
        skip(self),
        fields(otel.kind = "client", backend = "azure", path = %path.display(), recursive = _recursive)
    )]
    async fn mkdir(&self, path: &Path, _recursive: bool) -> BackendResult<()> {
        ...
        let object_path = match &self.prefix {
            ...
            None => ObjectPath::from(blob_name),
        };
    ...
    #[tracing::instrument(
        skip(self),
        fields(otel.kind = "client", backend = "azure", src = %src.display(), dest = %dest.display())
    )]
...
mod tests {
    ...
    fn in_memory_backend() -> AzureBackend {
        AzureBackend {
            store: Arc::new(object_store::memory::InMemory::new()),
            prefix: None,
        }
    ...
    async fn put_bytes(backend: &AzureBackend, path: &str, data: &[u8]) {
