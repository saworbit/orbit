...
mod tests {
    ...
    fn test_parse_s3_with_endpoint() {
        let (protocol, path) =
            parse_uri("s3://my-bucket/data.json?endpoint=http://localhost:9000").unwrap();
    ...
    fn test_parse_s3_with_region_and_endpoint() {
        let (protocol, path) =
            parse_uri("s3://my-bucket/dir/file?region=eu-west-1&endpoint=http://minio:9000")
                .unwrap();
    ...
    fn test_parse_s3_with_credentials() {
        let (protocol, path) =
            parse_uri("s3://AKIAIOSFODNN7EXAMPLE:wJalrXUtnFEMI/K7MDENG@my-bucket/data").unwrap();
