...
use std::path::Path;
use crate::audit::{AuditEntry, read_audit_log};
...
impl TransferStats {
    ...
    pub fn print(&self) {
        ...
        println!("   Total duration: {}", format_duration(self.total_duration_ms));
        ...
        if self.fastest_speed_bps > 0.0 {
            println!("   Fastest transfer: {}/s", format_bytes(self.fastest_speed_bps as u64));
        }
        if self.slowest_speed_bps > 0.0 {
            println!("   Slowest transfer: {}/s\n", format_bytes(self.slowest_speed_bps as u64));
        }
        
        if self.with_compression > 0 {
            println!("🗜️  Compression:");
            let pct = (self.with_compression as f64 / self.total_operations as f64) * 100.0;
            println!("   Operations with compression: {} ({:.1}%)", self.with_compression, pct);
            println!("   Average compression ratio: {:.1}%", self.total_compression_ratio);
            
            // Calculate space saved
            let original_size = self.total_bytes as f64 / (1.0 - self.total_compression_ratio / 100.0);
        ...
        if let Some(ref recent) = self.most_recent {
            ...
            println!("   {} in {}", format_bytes(recent.bytes_copied), format_duration(recent.duration_ms));
